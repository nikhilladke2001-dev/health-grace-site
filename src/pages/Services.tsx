...
import { Heart, Brain, Bone, Stethoscope, Baby, Eye, Pill, Activity } from "lucide-react";
...
export default function Services() {
  useEffect(() => { document.title = "Our Services — MediCare Hospital"; }, []);
  return (
    <main>
      <section className="bg-gradient-to-br from-hero-gradient-start to-hero-gradient-end py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-2 font-heading text-4xl font-extrabold text-white sm:text-5xl">Comprehensive Care</h1>
          <p className="mt-4 max-w-2xl text-lg text-white/80">Over 50 medical specialties under one roof, all working together to provide seamless, coordinated care.</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {departments.map((dept) => (
            <div key={dept.name} className="group rounded-xl border border-border bg-card p-6 transition-all hover:shadow-lg hover:-translate-y-1">
        ...
        <div className="mt-20 rounded-2xl bg-surface p-10 text-center sm:p-14">
          <h2 className="font-heading text-2xl font-bold text-foreground sm:text-3xl">Can't find what you're looking for?</h2>
          <p className="mx-auto mt-3 max-w-lg text-muted-foreground">We offer over 50 specialties. Contact us and our patient navigators will connect you with the right department.</p>
          <div className="mt-6"><Link to="/contact"><Button size="lg">Contact Us</Button></Link></div>
        </div>
      </section>
    </main>
  );
